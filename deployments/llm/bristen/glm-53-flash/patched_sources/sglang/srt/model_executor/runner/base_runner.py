    ...
    *,
    ...
    with torch.device(device):
        ...
        if pp_size > 1:
            ...
            # GLM-5.3 is a hybrid-channel model (hc_hidden_size set)
            # but still passes a SEPARATE residual between PP stages -- its
            # forward reads pp_proxy_tensors["residual"] unconditionally on
            # non-first ranks (glm5_next.py). The eager scheduler path
            # (scheduler_pp_mixin.py) always provisions residual too, so
            # match it here regardless of is_mhc. Only Kimi K3 supplies
            # num_blocks: its PP bank is token-major [T, blocks, H]; other
            # models keep the legacy [max_bs, H].
            residual_shape = (
