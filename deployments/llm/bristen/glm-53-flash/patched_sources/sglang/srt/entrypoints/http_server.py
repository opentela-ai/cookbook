...
def _freeze_gc_after_server_warmup(server_args: ServerArgs):
    ...
    # Job 83153: this POST raced uvicorn's accept loop right after bind and got
    # connection-refused. The unfrozen Tokenizer Manager then wedged its own
    # event loop under ~1M-object gen2 GC storms and the API never came up
    # (TCP connected but nothing was ever served). Retry for a while, then
    # fall back to freezing this process directly -- no HTTP involved.
    last_exc = None
    for _attempt in range(10):
        try:
            ...
            time.sleep(3)
    logger.warning(
        "post-warmup freeze_gc failed after retries (%s); freezing GC directly",
        last_exc,
    )
    try:
        from sglang.srt.managers.io_struct import FreezeGCReq
        from sglang.srt.utils.common import freeze_gc as _freeze_gc_local

        tm = getattr(_global_state, "tokenizer_manager", None)
        if tm is not None:
            # Best effort: tell the scheduler processes to freeze too.
            tm._dispatch_to_scheduler(FreezeGCReq())
        _freeze_gc_local("Tokenizer Manager (post-warmup fallback)")
    except Exception:
        logger.warning("direct GC freeze fallback failed", exc_info=True)
