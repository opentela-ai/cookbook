...
# ---------------------------------------------------------------------------
# SM80 (A100) vkernels bridge for the kpool index cache (vkernels #60, 1de4eec)
#
# On SM80, Triton cannot declare ``*fp8e4nv`` pointers, so every fp8-cache
# Triton kernel in this file JIT-fails on the first forward (bristen job
# 82822, ``ValueError: type fp8e4nv not supported in this architecture``).
#
# Strategy: keep the cache buffer EXACTLY as-is (uint8, per page
# [ssp x 128B fp8 K][ssp x 4B fp32 scale]) so the allocator, the
# deep_gemm/tilelang readers, cpu offload and move() all stay untouched, and
# replace only the fp8e4nv COMPUTE+STORE with: vkernels ``dsa_kpool`` kernels
# (bf16 math, no fp8e4nv anywhere) -> requantize to the legacy fp8+scale
# slots in torch. The requant replicates ``_hadamard_quantize_fp8`` + the
# Triton store offsets; the vkernels output already carries the same two bf16
...
# return_compressed path (CP / layer-shard only). Neither runs in the bristen
...
try:
    from vkernels import kernels as _vk_kernels

    _VK_DSA_KPOOL_ASSEMBLE = _vk_kernels.dsa_kpool_assemble
    _VK_DSA_KPOOL_DECODE = _vk_kernels.dsa_kpool_decode_update
...
def _vk_dsa_kpool_active(device: torch.device) -> bool:
    """True when the native bf16 kpool path should run on ``device``.

    SM80 (A100) is the target: the fp8 Triton kernels cannot JIT there.
    ``VKERNELS_DSA_KPOOL_FORCE=1`` forces the path on any device (used by
    the local CPU wiring test); ``VKERNELS_DSA_KPOOL_DISABLE=1`` vetoes it.
    """
    if _VK_DSA_KPOOL_ASSEMBLE is None:
        return False
    if os.environ.get("VKERNELS_DSA_KPOOL_DISABLE") == "1":
        return False
    if os.environ.get("VKERNELS_DSA_KPOOL_FORCE") == "1":
        return True
    if device.type == "cuda":
...
def _vk_np(t: torch.Tensor, dtype):
    import numpy as np
...
def _vk_bridge_requant_store(
    ...
    buf_numel = buf.shape[1]
    flat8 = buf.view(torch.float8_e4m3fn).reshape(-1)
    kidx = (pg * buf_numel).unsqueeze(1) + sp.unsqueeze(1) * INDEX_HEAD_DIM + torch.arange(
        INDEX_HEAD_DIM, device=dev
    )
    flat8[kidx] = kb.view(torch.float8_e4m3fn)
    flat32 = buf.view(torch.float32).reshape(-1)
    flat32[pg * (buf_numel // 4) + ssp * 32 + sp] = sb.view(torch.float32).squeeze(1)


def _vk_dsa_kpool_assemble_bridge(
    pool,
    buf: torch.Tensor,
    chunk_k: torch.Tensor,
    chunk_score: torch.Tensor,
    ...
    assert buf.dtype == torch.uint8
    assert buf.shape[1] == ssp * (INDEX_HEAD_DIM + 4)
    wm = None if write_mask is None else _vk_np(write_mask, torch.int32)
    scratch = np.zeros(num_pages * ssp * INDEX_HEAD_DIM, dtype=np.float32)
    _VK_DSA_KPOOL_ASSEMBLE(
        _vk_np(chunk_k, torch.float32),
        _vk_np(chunk_score, torch.float32),
        _vk_np(tail_k, torch.float32),
        _vk_np(tail_score, torch.float32),
        _vk_np(ape, torch.float32),
        _vk_np(req_pool_idx, torch.int32),
        _vk_np(n_from_tail, torch.int32),
        _vk_np(chunk_src_start, torch.int32),
        _vk_np(tail_logical_base, torch.int32),
        _vk_np(loc, torch.int32),
        slots_per_page=ssp,
        num_pages=num_pages,
        write_mask=wm,
        out=scratch,
    )
    rows = np.arange(n_pools, dtype=np.int64)
    if wm is not None:
        rows = rows[wm != 0]
    if rows.size == 0:
        return
    loc_l = _vk_np(loc, torch.int64)[rows]
    pages = loc_l // ssp
    sips = loc_l % ssp
    res = scratch.reshape(num_pages, ssp, INDEX_HEAD_DIM)
    x_host = res[pages, sips]
    _vk_bridge_requant_store(
        buf, num_pages, ssp, pages, sips, x_host, round_scale
    )


def _vk_dsa_kpool_decode_bridge(
    pool,
    buf: torch.Tensor,
    tail_k: torch.Tensor,
    tail_score: torch.Tensor,
    ...
    assert buf.shape[1] == ssp * (INDEX_HEAD_DIM + 4)
    scratch = np.zeros(num_pages * ssp * INDEX_HEAD_DIM, dtype=np.float32)
    tk = _vk_np(tail_k, torch.float32).copy()  # kernel updates tails in place
    ts = _vk_np(tail_score, torch.float32).copy()
    _VK_DSA_KPOOL_DECODE(
        _vk_np(key, torch.float32),
        _vk_np(slot_score, torch.float32),
        tk,
        ts,
        _vk_np(ape, torch.float32),
        _vk_np(block_tables, torch.int32),
        _vk_np(req_pool_indices, torch.int32),
        _vk_np(positions, torch.int32),
        _vk_np(seq_lens, torch.int32),
        _vk_np(out_cache_loc, torch.int32),
        tail_size=tail_size,
        slots_per_page=ssp,
        num_pages=num_pages,
        out=scratch,
    )
    tail_k.copy_(torch.from_numpy(tk).to(tail_k.device).to(tail_k.dtype))
    tail_score.copy_(torch.from_numpy(ts).to(tail_score.device).to(tail_score.dtype))
    pos_cpu = positions.detach().to("cpu", torch.int64)
    pos_l = pos_cpu.clamp(min=0)
    rpi_l = req_pool_indices.detach().to("cpu", torch.int64)
    valid = (
        (rpi_l >= 0)
        & (rpi_l < n_reqs)
        & (out_cache_loc.detach().to("cpu", torch.int64) != 0)
        & (pos_cpu >= 0)
        & (pos_cpu < seq_lens.detach().to("cpu", torch.int64))
    )
    written = valid & ((pos_cpu % pool_size) == pool_size - 1)
    if written.any():
        pool_id = pos_l // pool_size
        bt_row = (pool_id // ssp * pool_size).clamp(0, block_tables.shape[1] - 1)
        page = block_tables.detach().to("cpu", torch.int64)[
            torch.arange(batch), bt_row
        ]
        sip = pool_id % ssp
        sel = written.nonzero(as_tuple=True)[0]
        pages_np = page[sel].numpy()
        sips_np = sip[sel].numpy()
        res = scratch.reshape(num_pages, ssp, INDEX_HEAD_DIM)
        x_host = res[pages_np, sips_np]
        _vk_bridge_requant_store(
            buf, num_pages, ssp, pages_np, sips_np, x_host, round_scale
        )

def _vk_dsa_kpool_assemble_native(
    pool,
    buf: torch.Tensor,
    chunk_k: torch.Tensor,
    chunk_score: torch.Tensor,
    ...
    """bf16-cache assemble via vkernels #60.

    Same gated online-softmax + 128-pt Hadamard math as the Triton kernel,
    minus the fp8 quantize: the rotated mean is stored bf16 directly and the
    ...
    assert buf.shape[1] == ssp * INDEX_HEAD_DIM
    wm = None if write_mask is None else _vk_np(write_mask, torch.int32)
    scratch = np.zeros(num_pages * ssp * INDEX_HEAD_DIM, dtype=np.float32)
    _VK_DSA_KPOOL_ASSEMBLE(
        _vk_np(chunk_k, torch.float32),
    ...
    rows = torch.from_numpy(np.arange(n_pools, dtype=np.int64))
    if wm is not None:
        rows = rows[torch.from_numpy(wm) != 0]
    if rows.numel() == 0:
        return
    loc_l = loc.detach().to("cpu", torch.int64)[rows]
    res = torch.from_numpy(scratch).view(num_pages, ssp, INDEX_HEAD_DIM)
    page, sip = loc_l // ssp, loc_l % ssp
    buf.view(num_pages, ssp, INDEX_HEAD_DIM)[page, sip] = res[
        page, sip
    ].to(buf.dtype).to(buf.device)


def _vk_dsa_kpool_decode_native(
    pool,
    buf: torch.Tensor,
    tail_k: torch.Tensor,
    tail_score: torch.Tensor,
    ...
    """bf16-cache decode update via vkernels #60.

    Mirrors the Triton kernel: pool-complete rows (``pos % pool_size ==
    ...
    tail_k.copy_(torch.from_numpy(tk).to(tail_k.device).to(tail_k.dtype))
    tail_score.copy_(torch.from_numpy(ts).to(tail_score.device).to(tail_score.dtype))
    pos_l = positions.detach().to("cpu", torch.int64).clamp(min=0)
    rpi_l = req_pool_indices.detach().to("cpu", torch.int64)
    valid = (
        (rpi_l >= 0)
        & (rpi_l < n_reqs)
        & (out_cache_loc.detach().to("cpu", torch.int64) != 0)
        & (positions.detach().to("cpu", torch.int64) >= 0)
        & (positions.detach().to("cpu", torch.int64) < seq_lens.detach().to("cpu", torch.int64))
    )
    written = valid & ((pos_l % pool_size) == pool_size - 1)
    if written.any():
        pool_id = pos_l // pool_size
        bt_row = (pool_id // ssp * pool_size).clamp(
        ...
        res = torch.from_numpy(scratch).view(num_pages, ssp, INDEX_HEAD_DIM)
        sel = written.nonzero(as_tuple=True)[0]
        buf.view(num_pages, ssp, INDEX_HEAD_DIM)[
            page[sel], sip[sel]
        ] = res[page[sel], sip[sel]].to(buf.dtype).to(buf.device)
...
def kpool_decode_update_and_maybe_write_cache(
    ...
    if buf.dtype == torch.uint8 and _vk_dsa_kpool_active(key.device):
        # SM80: fp8e4nv Triton JIT unsupported -> vkernels + fp8 requant store
        _vk_dsa_kpool_decode_bridge(
            pool, buf, tail_k, tail_score, key, slot_score, ape,
            block_tables, req_pool_indices, positions, seq_lens,
            out_cache_loc, round_scale,
        )
        return
    if buf.dtype == torch.bfloat16 and _vk_dsa_kpool_active(key.device):
        # upstream bf16-cache layout (vkernels#60 follow-up)
        _vk_dsa_kpool_decode_native(
            pool, buf, tail_k, tail_score, key, slot_score, ape,
            block_tables, req_pool_indices, positions, seq_lens,
            out_cache_loc,
        )
...
def kpool_assemble_softmax_rotate_write_cache(
    ...
    if buf.dtype == torch.uint8 and _vk_dsa_kpool_active(chunk_k.device):
        # SM80: fp8e4nv Triton JIT unsupported -> vkernels + fp8 requant store
        _vk_dsa_kpool_assemble_bridge(
            pool, buf, chunk_k, chunk_score, tail_k, tail_score,
            req_pool_idx, n_from_tail, chunk_src_start, tail_logical_base,
            ape, loc, write_mask, round_scale,
        )
        return
    if buf.dtype == torch.bfloat16 and _vk_dsa_kpool_active(chunk_k.device):
        # upstream bf16-cache layout (vkernels#60 follow-up)
        _vk_dsa_kpool_assemble_native(
            pool, buf, chunk_k, chunk_score, tail_k, tail_score,
            req_pool_idx, n_from_tail, chunk_src_start, tail_logical_base,
            ape, loc, write_mask,
        )
