#!/usr/bin/env bash
# Register the running Qwen/Qwen3.8-27B-FP8 engine as the `llm` service on the
# OpenTela network (eth-easl/OpenTela v0.2.3 / arm64). Sidecar topology:
# the engine (sglang in the qwen38-dgx-spark container, --network host) is
...
#
#   engine container (qwen38-dgx-spark, --network host)  :${SERVE_PORT}  <-- /health = 200
#        ^
...
#
# Usage:
#   bash register_qwen38_otela.sh          # start (foreground)
#   bash register_qwen38_otela.sh daemon   # start (background, logs to file)
#   bash register_qwen38_otela.sh stop     # stop the daemon
#   bash register_qwen38_otela.sh status   # check the daemon
set -uo pipefail
...
SERVED_MODEL_ID="${SERVED_MODEL_ID:-Qwen/Qwen3.8-27B-FP8}"   # org/model-name form; see ../../../../../conventions/
...
# ds6 (qwen36 recipe): --seed 0, 1, and 2 all yield the SAME peer ID (keys/id
# fixes it); deleting $CFG_DIR/keys and re-initing yields a DIFFERENT peer ID.
# The Solana provider key at ~/.config/opentela/ is separate and not affected
# by --config-dir. --seed is vestigial and passed only for forward-compat.
...
[ -x "${OTELA_BIN}" ] || { echo "FATAL: otela binary not found/executable: ${OTELA_BIN}" >&2
  ...
  echo "       On this host the sibling qwen36 recipe already has it:" >&2
  echo "         cp ${RECIPE_DIR}/../qwen36-35b-a3b/run/otela/otela ${OTELA_BIN}" >&2
  echo "       (or: OTELA_BIN=${RECIPE_DIR}/../qwen36-35b-a3b/run/otela/otela bash $0 daemon)" >&2
  exit 1; }
mkdir -p "${CFG_DIR}"
